...
namespace larg4{
  ...
  //----------------------------------------------------------------------------
  void ElectronDriftAlg::PrecalculateSC(std::vector<sim::SimEDep> const& edeps)
  {
    fSCCalcMap.clear();
    for(auto const& edep : edeps){
      auto index = GetSCMapIndex(edep.xpos,edep.ypos,edep.zpos);
      if(fSCCalcMap[index].size()==0){
	double x = (double)((unsigned long)edep.xpos) + 0.5;
	double y = (double)((unsigned long)edep.ypos) + 0.5 - (fGeo->DetHalfHeight());
	double z = (double)((unsigned long)edep.zpos) + 0.5;

	fSCE->GetPosOffsets(edep.xpos,edep.ypos,edep.zpos,fPosOffsets);	
	fSCCalcMap[index].push_back(fPosOffsets);
	fSCE->GetEfieldOffsets(edep.xpos,edep.ypos,edep.zpos,fPosOffsets);	
	fSCCalcMap[index].push_back(fPosOffsets);
      }
    }
  ...
  //----------------------------------------------------------------------------
  void ElectronDriftAlg::DriftElectrons(sim::SimEDep const& edep)
  {
    ...
    //get the right TPC
    fXYZ[0] = edep.xpos; fXYZ[1] = edep.ypos; fXYZ[2] = edep.zpos; 
    ...
    /*
    //if space charge, get position offsets
    if (fSCE->EnableSimSpatialSCE())
      fSCE->GetPosOffsets(edep.xpos,edep.ypos,edep.zpos,fPosOffsets);
    else
      fPosOffsets.resize(3,0.0);
    */
    fPosOffsets = fSCCalcMap[GetSCMapIndex(edep.xpos,edep.ypos,edep.zpos)][0];
    auto t4 = std::chrono::high_resolution_clock::now();

    //determine xdrift distance
    if(tpcgeo.DriftDirection() == geo::kNegX)
      fXDrift = edep.xpos - tpcgeo.PlaneLocation(0)[0] - fPosOffsets[0];
    else if(tpcgeo.DriftDirection() == geo::kNegX)
      fXDrift = tpcgeo.PlaneLocation(0)[0] - edep.xpos - fPosOffsets[0];
    if(fXDrift<0)
      return;
    ...
    randGauss.fireArray(fNElectronClusters, &fYDiff[0], edep.ypos+fPosOffsets[1],fTransverseDiffSigma);
    randGauss.fireArray(fNElectronClusters, &fZDiff[0], edep.zpos+fPosOffsets[2],fTransverseDiffSigma);
    ...
    for(size_t i_cl=0; i_cl < fNElectronClusters; ++i_cl)
      fTDiff[i_cl] = fTDrift + fXDiff[i_cl]*fRecipDriftVelocityVolume + edep.time;
