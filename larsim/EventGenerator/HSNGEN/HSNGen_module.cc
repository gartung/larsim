...
namespace evgen{
      ...
      fRandomXZShift(p.get< double >("RandomXZShift",0.))
  {
    ...
    if(fShowerInputFiles.size() != fShowerFluxConstants.size() || fShowerInputFiles.size()==0 || fShowerFluxConstants.size()==0)
      throw cet::exception("HSNGen") << "ShowerInputFiles and ShowerFluxConstants have different or invalid sizes!"<<"\n";
    fShowerInputs=fShowerInputFiles.size();
    
    if(fSampleTime==0.) throw cet::exception("HSNGen") << "SampleTime not set!";
    
    if(fProjectToHeight==0.) mf::LogInfo("HSNGen")<<"Using 0. for fProjectToHeight!"
    ;
  ...
  void HSNGen::ProjectToBoxEdge(	const double 	xyz[],
                                        const double 	indxyz[],
                                        const double 	xlo,
                                        const double 	xhi,
                                        const double 	ylo,
  ...
  void HSNGen::openDBs(){
    ...
    if ( ifdh_debug_env ) {
      mf::LogInfo("HSNGen") << "IFDH_DEBUG_LEVEL: " << ifdh_debug_env<<"\n";
    ...
    for(int i=0; i<fShowerInputs; i++){
      if(fShowerInputFiles[i].find("*")==std::string::npos){ 
        ...
        mf::LogInfo("HSNGen") << "Selected"<<selectedflist.back().first<<"\n";
	  }else{
		...
		if(flist.size()==1){ //0th element is the search path:pattern
			...
			throw cet::exception("HSNGen") << "No files returned for path:pattern: "<<path<<":"<<pattern<<std::endl;
		}
		selectedflist.push_back(flist[selIndex]);
		mf::LogInfo("HSNGen") << "For "<<fShowerInputFiles[i]<<":"<<pattern
    ...
    for(unsigned int i=0; i<selectedflist.size(); i++){
      ...
      LOG_DEBUG("HSNGen") << "Fetched; local path: "<<fetchedfile;
    ...
    for(unsigned int i=0; i<locallist.size(); i++){
      ...
      if (res!= SQLITE_OK)
        throw cet::exception("HSNGen") << "Error opening db: (" <<locallist[i].c_str()<<") ("<<res<<"): " << sqlite3_errmsg(fdb[i]) << "; memory used:<<"<<sqlite3_memory_used()<<"/"<<sqlite3_memory_highwater(0)<<"\n";
      else
        mf::LogInfo("HSNGen")<<"Attached db "<< locallist[i]<<"\n";
    }
  }

  double HSNGen::wrapvar( const double var, const double low, const double high){
    //wrap variable so that it's always between low and high
    return (var - (high - low) * floor(var/(high-low))) + low;
  }

  double HSNGen::wrapvarBoxNo( const double var, const double low, const double high, int& boxno){
    //wrap variable so that it's always between low and high
    boxno=int(floor(var/(high-low)));
    return (var - (high - low) * floor(var/(high-low))) + low;
  }
  
  void HSNGen::populateTOffset(){
    ...
    for(int i=0; i<fShowerInputs; i++){
        ...
        if ( sqlite3_prepare(fdb[i], kStatement.c_str(), -1, &statement, 0 ) == SQLITE_OK ){
          ...
          if ( res == SQLITE_ROW ){
            ...
            mf::LogInfo("HSNGen")<<"For showers input "<< i<<" found particles.min(t)="<<t<<"\n";
            if (i==0 || t<fToffset_corsika) fToffset_corsika=t;
          }else{
            throw cet::exception("HSNGen") << "Unexpected sqlite3_step return value: (" <<res<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
          }         
        }else{
          throw cet::exception("HSNGen") << "Error preparing statement: (" <<kStatement<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
        }
    }
    
    mf::LogInfo("HSNGen")<<"Found corsika timeoffset [ns]: "<< fToffset_corsika<<"\n";
  }
  
  void HSNGen::populateNShowers(){
    ...
    for(unsigned int c = 0; c < geom->Ncryostats(); ++c){
      ...
      for (unsigned int bnd = 0; bnd<6; bnd++){
        mf::LogVerbatim("HSNGen")<<"Cryo Boundary: "<<bnd<<"="<<bounds[bnd]<<" ( + Buffer="<<fBuffBox[bnd]<<")\n";
    ...
    */
    ...
    for(int i=0; i<fShowerInputs; i++){
      ...
      //  double thisrnd=flat();//need a new random number for each query
        if ( sqlite3_prepare(fdb[i], kStatement.c_str(), -1, &statement, 0 ) == SQLITE_OK ){
          ...
          if ( res == SQLITE_ROW ){
            ...
            mf::LogVerbatim("HSNGen")<<"For showers input "<< i<<" found e_hi="<<upperLimitOfEnergyRange<<", e_lo="<<lowerLimitOfEnergyRange<<", slope="<<energySlope<<", k="<<fShowerFluxConstants[i]<<"\n";
          }else{
            throw cet::exception("HSNGen") << "Unexpected sqlite3_step return value: (" <<res<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
          }         
        }else{
      ...
      mf::LogVerbatim("HSNGen")<<"For showers input "<< i
                               <<" the number of showers per event is "<<(int)NShowers<<"\n";
    }
  }
  
  void HSNGen::GetSample(simb::MCTruth& mctruth){
    ...
    for(int i=0; i<fShowerInputs; i++){
      ...
      mf::LogInfo("HSNGen") << " Shower input " << i << " with mean " << fNShowersPerEvent[i] << " generating " << nShowerCntr;
      ...
      while(nShowerCntr>0){
        ...
        LOG_DEBUG("HSNGen")<<"Executing: "<<kthisStatement;
        if ( sqlite3_prepare(fdb[i], kthisStatement.Data(), -1, &statement, 0 ) == SQLITE_OK ){
          ...
          throw cet::exception("HSNGen") << "Error preparing statement: (" <<kthisStatement<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
        }
  ...
  void HSNGen::produce(art::Event& evt){
    ...
    GetSample(pretruth);
    mf::LogInfo("HSNGen")<<"GetSample number of particles returned: "<<pretruth.NParticles()<<"\n";
    ...
    mf::LogInfo("HSNGen")<<"Number of particles from getsample crossing cryostat + bounding box: "<<truth.NParticles()<<"\n";
