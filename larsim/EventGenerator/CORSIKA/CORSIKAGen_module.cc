...
namespace evgen{
  ...
  CORSIKAGen::CORSIKAGen(fhicl::ParameterSet const& p)
    : EDProducer{p},
      ...
      fShowerCopyType(p.get< std::string >("ShowerCopyType", "PNFS")), 
  ...
  void CORSIKAGen::openDBs(std::string const& module_label){
    ...
    for(int i=0; i<fShowerInputs; i++){
      if(fShowerInputFiles[i].find("*")==std::string::npos){
        ...
        // default to PNFS approach when wildcards
    ...
    for(unsigned int i=0; i<selectedflist.size(); i++){
      ...
      if (fShowerCopyType == "PNFS") {
      	std::string fetchedfile(fIFDH->fetchInput(selectedflist[i].first));
     	MF_LOG_DEBUG("CorsikaGen") << "Fetched; local path: "<<fetchedfile << "; copy type: PNFS";
      	locallist.push_back(fetchedfile);
      }
      else if (fShowerCopyType == "LOCAL") {
      	std::string fetchedfile(selectedflist[i].first);
     	MF_LOG_DEBUG("CorsikaGen") << "Fetched; local path: "<<fetchedfile << "; copy type: LOCAL";
      	locallist.push_back(fetchedfile);
      }
      else throw cet::exception("CORSIKAGen") << "Error copying shower db: ShowerCopyType must be \"PNFS\" or \"LOCAL\"\n";
    }
