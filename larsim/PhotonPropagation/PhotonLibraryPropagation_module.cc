...
private:
  ...
  double GetScintYield(sim::SimEDep const&, detinfo::LArProperties const&);
  ...
  void PrecalculateSC(std::vector<sim::SimEDep> const&);
...
double phot::PhotonLibraryPropagation::GetScintYield(sim::SimEDep const& edep,
...
void phot::PhotonLibraryPropagation::produce(art::Event & e)
{
  ...
  auto const& edep_handle = e.getValidHandle< std::vector<sim::SimEDep> >(fEDepTag);
  ...
  for(auto const& edep : edeps){
    ...
    double const xyz[3] = { edep.xpos, edep.ypos, edep.zpos };
...
//----------------------------------------------------------------------------
void phot::PhotonLibraryPropagation::PrecalculateSC(std::vector<sim::SimEDep> const& edeps)
{
  ...
  for(auto const& edep : edeps){
    ...
    if(fSCCalcMap[index].size()==0){
      ...
      double y = (double)((unsigned long)edep.ypos) + 0.5 - (geo.DetHalfHeight());
