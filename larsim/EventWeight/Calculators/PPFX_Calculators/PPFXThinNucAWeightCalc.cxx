...
namespace evwgh {
  class PPFXThinNucAWeightCalc : public WeightCalc
  {
     public:
       PPFXThinNucAWeightCalc();
     ...
     DECLARE_WEIGHTCALC(PPFXThinNucAWeightCalc)
  };
  
  PPFXThinNucAWeightCalc::PPFXThinNucAWeightCalc()
  {
  }

  void PPFXThinNucAWeightCalc::Configure(fhicl::ParameterSet const& p, CLHEP::HepRandomEngine&)
  {
    ...
    fGaussRandom = new CLHEP::RandGaussQ(rng->getEngine(art::ScheduleID::first(), std::string("eventweight"), std::string("ppfx_thinna")));    
  ...
  std::vector<std::vector<double> > PPFXThinNucAWeightCalc::GetWeight(art::Event & e)
  {
	...
	for(unsigned int iuniv=0;iuniv<vttnucleona.size();iuniv++){
	  tmp_vhptot.push_back(float(vttnucleona[iuniv]));
	}
  ...
  REGISTER_WEIGHTCALC(PPFXThinNucAWeightCalc)
}
