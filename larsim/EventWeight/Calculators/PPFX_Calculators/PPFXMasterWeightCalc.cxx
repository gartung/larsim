// Class that runs PPFX in a master weight mode. Changes added by Krishan Mistry
// Email: kmistry@fnal.gov 
...
namespace evwgh {
    class PPFXMasterWeightCalc : public WeightCalc
    {
        public:
            PPFXMasterWeightCalc();
        ...
        private:
            ...
            DECLARE_WEIGHTCALC(PPFXMasterWeightCalc)
    };

    PPFXMasterWeightCalc::PPFXMasterWeightCalc()
    {
    }

    void PPFXMasterWeightCalc::Configure(fhicl::ParameterSet const& p, CLHEP::HepRandomEngine&)
    {
        ...
        fGaussRandom = new CLHEP::RandGaussQ(rng->getEngine(art::ScheduleID::first(), std::string("eventweight"), std::string("ppfx_master")));    
    ...
    std::vector<std::vector<double> > PPFXMasterWeightCalc::GetWeight(art::Event & e)
    {
        ...
        while ( ( flag = mcitr.Next() ) ) {
            ...
            if (fMode=="reweight") {
                ...
                std::vector<double> vtotal       = fPPFXrw->GetTotalWeights();

                std::vector<double> tmp_vhptot;
                for(unsigned int iuniv=0;iuniv<vtotal.size();iuniv++){
                    tmp_vhptot.push_back(float(vtotal[iuniv]));
                }
    ...
    REGISTER_WEIGHTCALC(PPFXMasterWeightCalc)
}
